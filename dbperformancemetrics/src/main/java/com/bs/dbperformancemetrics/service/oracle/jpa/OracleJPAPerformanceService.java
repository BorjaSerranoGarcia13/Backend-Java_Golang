...
public class OracleJPAPerformanceService implements IDatabasePerformanceService {
    ...
    public List<PerformanceResult> findUserByUsername() {
        ...
        final int initComparison = 1;
        final int middleComparison = Constants.NUMBER_OF_DATA / 2;

        final long executionTimeBeginning = PerformanceMeasurementUtils.measureExecutionTime(() -> {
            userService.findByUsername("username" + initComparison);
        });

        final long executionTimeMiddle = PerformanceMeasurementUtils.measureExecutionTime(() -> {
            userService.findByUsername("username" + middleComparison);
        });
    ...
    public List<PerformanceResult> findUserByName() {
        ...
        final int initPos = 1;
        final int middlePos = Constants.NUMBER_OF_DATA / 2;

        final long executionTimeBeginning = PerformanceMeasurementUtils.measureExecutionTime(() -> {
            userService.findByName("username" + initPos);
        });

        final long executionTimeMiddle = PerformanceMeasurementUtils.measureExecutionTime(() -> {
            userService.findByName("username" + middlePos);
        });
    ...
    public List<PerformanceResult> findPasswordByUsername() {
        ...
        final long executionTimeBeginning = PerformanceMeasurementUtils.measureExecutionTime(() -> {
            userService.findPasswordByUsername("username" + initPos);
        });

        final long executionTimeMiddle = PerformanceMeasurementUtils.measureExecutionTime(() -> {
            userService.findPasswordByUsername("username" + middlePos);
        });
